...
if config.write_back_log:
    f_back_log = open(path.relpath(
        config.back_log_path + '/' + config.insName + '_' + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")) + '.log',
                      'a');
time = 0
times = list()
last_ask = 0
...
def process_data(ask, bid, status):
    ...
    if config.write_back_log:
        ...
        datetime.datetime.now(), config.insName, pReq.response.get('prices')[0].get('asks')[1].get('price'),
        pReq.response.get('prices')[0].get('bids')[1].get('price'), pChange, ask - bid, result))
...
def do_close_long():
    try:
        r = positions.PositionClose(config.account_id, 'sber', {"longUnits": "ALL"})
        resp = oanda.request(r)
        print(resp)
